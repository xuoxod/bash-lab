...
class ThreadedNmapScanner:
    ...
    def __init__(
        ...
        self.scan_results: Dict[str, Tuple[str, List[Dict], str]] = {}
    ...
    def scan_type(self, scan_type: str):
        """Sets the scan type, validating the input."""
    ...
    def _execute_nmap_scan(self, target: str) -> str:  # Returns XML output
        ...
        # Extend the command with the scan type arguments
        command.extend(self.SCAN_TYPES[self.scan_type]["arguments"])
                    ...
                    if output:  # Only process if there's XML output
                        try:
                            ...
                            if target not in self.scan_results:
                                ...
                                existing_hostname, existing_ports, _ = (
                                    self.scan_results[target]
                                )
                                self.scan_results[target] = (
                                    existing_hostname,
                            ...
                            self.print_error(
                                f"Error processing results for {target}: {e}"
                            )
        ...
        try:
            with open(self.csv_filename, "a", newline="") as csvfile:
                ...
                for target, (
                    ...
                    open_ports,
                    os_info,
                ) in self.scan_results.items():  # Unpack os_info
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: python3 tns.py -t 192.168.1.1,192.168.1.10-192.168.1.20,192.168.1.0/24 -p 80,443,8080 -s 12",
