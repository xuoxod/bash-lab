...
int connect_to_device(const char *mac_address, int *channel) {
    ...
    // Allocate a socket
    printf(BRIGHT_YELLOW "   Creating socket...\n" ANSI_COLOR_RESET);
    ...
    printf(BRIGHT_YELLOW "   Setting connection parameters...\n" ANSI_COLOR_RESET);
    addr.rc_family = AF_BLUETOOTH;
    str2ba(mac_address, &addr.rc_bdaddr);

    // Try connecting to channels 1-30
    for (*channel = 1; *channel <= 30; (*channel)++) {
    ...
    // If no connection was successful, close the socket and return -1
    close(sock);
    return -1;
}
...
int main(int argc, char *argv[]) {
    ...
    if (sock < 0) {
        fprintf(stderr, ANSI_COLOR_RED "Error: " ANSI_COLOR_RESET "Failed to connect to device on any channel.\n");
